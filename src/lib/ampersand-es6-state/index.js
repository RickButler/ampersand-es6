...
class State extends Events {
    ...
    _createPropertyDefinition(name, desc, isSession) {
    ...
    _getDerivedProperty(name, flushCache) {
    ...
    _getCachedEventBubblingHandler(propertyName) {
    ...
    _safeSetsafeSet(property, value) {
        if (property in this) {
...
var dataTypes = {
    ...
    state: {
        set(newVal) {
            var isInstance = newVal instanceof State || (newVal && newVal.isState);
            if (isInstance) {
                return {
                    val: newVal,
                    type: 'state'
        ...
        onChange(newVal, previousVal, attributeName) {
